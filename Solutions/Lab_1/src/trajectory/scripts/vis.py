...
def pose_listener( data):
    global line_poses, line_poses2, line_poses2, X_track, Y_track, tracking_points, pose
...
def process():
    ...
    while not rospy.is_shutdown():
        ...
        tracking_points.append([pose[0], pose[1]])
        rate.sleep()
...
if __name__ == '__main__':
    try:
        ...
        for w in W:
            ax.plot(w[0],w[1],'bs')
        ax.plot(w[0], w[1], 'bs', ms=4, label="Waypoints")
        X=[]
        Y=[]
        for pt in tracking_points:
            X.append(pt[0])
            Y.append(pt[1])
        ax.plot(X, Y, 'r', lw=2, alpha=0.7, label='trajectory')
